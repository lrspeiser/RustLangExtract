...
//!
//! New in this version:
//! - --passes / --vote (union|majority) with --majority-threshold
//! - --second-pass consolidate (cross-chunk dedupe/merge)
//! - --adaptive-rechunk (auto zoom-in on boundary hot spots)
//! - --validator (regex/domain checks) + --validator-retry (LLM repair)
//!
//! Pipeline:
//!  input.txt -> chunk -> (N passes per chunk + vote) -> merge -> [optional] hot-spot re-chunk
//!  -> [optional] cross-chunk consolidate -> [optional] validate/repair -> Parquet + JSONL + HTML
//!
//! Design goals:
//! - SAFE: memory-safe chunking (Rope), strict schema outputs, explicit retries/timeouts
//! - AUDITABLE: every extraction has exact source spans (start_char/end_char)
//! - ANALYTICS-READY: writes Parquet for instant DuckDB/Polars
//!
//! Logging is VERY verbose so you can follow every step.
...
struct Cli {
    ...
    // ---------- NEW FLAGS ----------
    /// Number of passes per chunk (self-consistency). N>1 enables voting/union.
    #[arg(long, default_value_t=1)]
    passes: usize,

    /// Voting strategy across passes: union | majority
    #[arg(long, default_value="union")]
    vote: String,

    /// Majority threshold K (only used with --vote=majority)
    #[arg(long, default_value_t=2)]
    majority_threshold: usize,

    /// Enable cross-chunk consolidation (global dedupe/merge)
    #[arg(long, default_value_t=false)]
    second_pass: bool,

    /// Enable validator pass (regex/domain checks) and optional LLM repair
    #[arg(long, default_value_t=false)]
    validator: bool,

    /// If validator finds issues, attempt a single LLM repair call on the local span context
    #[arg(long, default_value_t=false)]
    validator_retry: bool,

    /// Enable adaptive re-chunking for hot spots near boundaries
    #[arg(long, default_value_t=false)]
    adaptive_rechunk: bool,

    /// Percent (0-100) of extractions near boundaries that triggers hot-spot re-chunk
    #[arg(long, default_value_t=35)]
    hotspot_threshold: u8,

    /// Re-chunk size to use for hot spots
    #[arg(long, default_value_t=4000)]
    rechunk_size: usize,

    /// Re-chunk overlap to use for hot spots
    #[arg(long, default_value_t=800)]
    rechunk_overlap: usize,
}
...
impl VoteStrategy {
    fn from_str(s: &str) -> Self {
        match s.to_ascii_lowercase().as_str() {
            "majority" => VoteStrategy::Majority,
            _ => VoteStrategy::Union,
        }
    }
}
...
pub struct Extraction {
    ...
    pub start_char: usize, // global
    pub end_char: usize,   // global (exclusive)
...
/// Returns Vec of (chunk_text, global_start_offset, chunk_id).
...
// Helper for extracting local context snippet
    ...
    loop {
        ...
        let body = json!({
            ...
            "input": [
              ...
              { "role": "user",   "content": [{ "type":"input_text", "text": example_text }] },
        ...
        info!("➡️ [OpenAI] Sending structured request (attempt {attempt})");
        ...
        if status.is_success() {
            info!("✅ [OpenAI] OK (network={}ms, decode={}ms)", send_elapsed.as_millis(), decode_elapsed.as_millis());
            return Ok(val);
        }
...
fn extract_json_payload(val: &serde_json::Value) -> Result<serde_json::Value> {
        ...
        if let Some(first) = arr.first() {
            if let Some(text) = first.get("text").and_then(|t| t.as_str()) {
                let parsed: serde_json::Value =
                    serde_json::from_str(text).context("Failed to parse model's output_text as JSON")?;
                return Ok(parsed);
            }
        }
    }
    ...
    Err(anyhow::anyhow!("Could not find JSON matching schema in model response"))
}
...
fn parse_batch_from_payload(
    ...
    let mut batch: ExtractionBatch = if payload.get("extractions").is_some() {
        ...
        let exts: Vec<Extraction> = serde_json::from_value(payload.clone())
        ...
        let exts_v = payload.get("extractions").and_then(|v| v.as_array()).cloned().unwrap_or_default();
        let exts: Vec<Extraction> = exts_v.into_iter().map(|e| serde_json::from_value(e).unwrap()).collect();
        ExtractionBatch {
            document_id: document_id.to_string(),
            chunk_id: chunk_id.to_string(),
            extractions: exts,
...
fn write_parquet<P: AsRef<Path>>(batches: &[ExtractionBatch], path: P) -> Result<()> {
    ...
    // One encoding vector per column (flat schema).
    let encodings: Vec<Vec<pq::Encoding>> = schema.fields.iter().map(|_| vec![pq::Encoding::Plain]).collect();
